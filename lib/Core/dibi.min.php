...
if (version_compare(PHP_VERSION, '5.2.0', '<')) {
	throw
	new
	Exception('dibi needs PHP 5.2.0 or newer.');
}
...
DibiDateTime
	...
	function
	__wakeup() {
		$this->__construct($this->fix[0], new
		DateTimeZone($this->fix[1]));
		unset($this->fix);
	}
	...
	setTimestamp($timestamp) {
		return $this->__construct(date('Y-m-d H:i:s', $timestamp), new
		DateTimeZone($this->getTimezone()->getName()));
	}
...
DibiObject {
	...
	__call($name, $args) {
		...
		if ($name === '') {
			throw
			new
			LogicException("Call to class '$class' method without name.");
		}
		...
		throw
		new
		LogicException("Call to undefined method $class::$name().");
	}
	...
	__callStatic($name, $args) {
		...
		throw
		new
		LogicException("Call to undefined static method $class::$name().");
	}
	...
	function&__get($name) {
		...
		if ($name === '') {
			throw
			new
			LogicException("Cannot read a class '$class' property without name.");
		}
		$name[0] = $name[0] & "\xDF";
		$m = 'get' . $name;
		...
		throw
		new
		LogicException("Cannot read an undeclared property $class::\$$name.");
	}
	...
	__set($name, $value) {
		...
		if ($name === '') {
			throw
			new
			LogicException("Cannot assign to a class '$class' property without name.");
		}
		$name[0] = $name[0] & "\xDF";
		if (self::hasAccessor($class, 'get' . $name) || self::hasAccessor($class, 'is' . $name)) {
			...
			if (self::hasAccessor($class, $m)) {
				...
				throw
				new
				LogicException("Cannot assign to a read-only property $class::\$$name.");
			}
		}
		$name = func_get_arg(0);
		throw
		new
		LogicException("Cannot assign to an undeclared property $class::\$$name.");
	}
	...
	__unset($name) {
		...
		throw
		new
		LogicException("Cannot unset the property $class::\$$name.");
	}
...
DibiHashMap
	...
	__get($nm) {
		if ($nm == '') {
			...
			return
				isset($this->$nm) ? $this->$nm : $this->$nm = call_user_func($this->getCallback(), '');
		} else {
...
DibiConnection
	...
	__construct($config, $name = null) {
		...
		if (is_string($config)) {
			...
			foreach ($config
				...
				$tmp[$key] = $val
				instanceof
				Traversable ? iterator_to_array($val) : $val;
			}
			$config = $tmp;
		} elseif (!is_array($config)) {
			throw
			new
			InvalidArgumentException('Configuration must be array, string or object.');
		}
		...
		if (!empty($profilerCfg['run'])) {
			...
			if (isset($profilerCfg['file'])) {
				$this->onEvent[] = array(new
				DibiFileLogger($profilerCfg['file'], $filter), 'logEvent');
			}
			if (DibiFirePhpLogger::isAvailable()) {
				$this->onEvent[] = array(new
				DibiFirePhpLogger($filter), 'logEvent');
			}
			if (class_exists('DibiNettePanel', false)) {
				$panel = new
				DibiNettePanel(isset($profilerCfg['explain']) ? $profilerCfg['explain'] : true, $filter);
				$panel->register($this);
			}
		}
		$this->substitutes = new
		DibiHashMap(create_function('$expr', 'return ":$expr:";'));
	...
	function
	connect() {
		$event = $this->onEvent ? new
		DibiEvent($this, DibiEvent::CONNECT) : null;
		try {
	...
	dataSource($args) {
		...
		return
			new
			DibiDataSource($this->translateArgs($args), $this);
	}
	...
	nativeQuery($sql) {
		...
		dibi::$sql = $sql;
		$event = $this->onEvent ? new
		DibiEvent($this, DibiEvent::QUERY, $sql) : null;
		try {
	...
	getAffectedRows() {
		...
		if (!is_int($rows) || $rows < 0) throw
		new
		DibiException('Cannot retrieve number of affected rows.');
	...
	getInsertId($sequence = null) {
		...
		if ($id < 1) throw
		new
		DibiException('Cannot retrieve last generated ID.');
	...
	begin($savepoint = null) {
		$this->connected || $this->connect();
		$event = $this->onEvent ? new
		DibiEvent($this, DibiEvent::BEGIN, $savepoint) : null;
		try {
	...
	commit($savepoint = null) {
		$this->connected || $this->connect();
		$event = $this->onEvent ? new
		DibiEvent($this, DibiEvent::COMMIT, $savepoint) : null;
		try {
	...
	rollback($savepoint = null) {
		$this->connected || $this->connect();
		$event = $this->onEvent ? new
		DibiEvent($this, DibiEvent::ROLLBACK, $savepoint) : null;
		try {
	...
	createResultSet(IDibiResultDriver $resultDriver) {
		$res = new
		DibiResult($resultDriver);

		return $res->setFormat(dibi::DATE, $this->config['result']['formatDate'])->setFormat(dibi::DATETIME, $this->config['result']['formatDateTime']);
	}
	...
	update($table, $args) {
		if (!(is_array($args) || $args
			...
			throw
			new
			InvalidArgumentException('Arguments must be array or Traversable.');
		}
	...
	substitute($value) {
		return
			strpos($value, ':') === false ? $value : preg_replace_callback('#:([^:\s]*):#', array($this, 'subCb'), $value);
	}
	...
	loadFile($file) {
		...
		if (!$handle) {
			throw
			new
			RuntimeException("Cannot open file '$file'.");
		}
		$count = 0;
		$sql = '';
		while (!feof($handle)) {
	...
	getDatabaseInfo() {
		...
		return
			new
			DibiDatabaseInfo($this->driver->getReflector(), isset($this->config['database']) ? $this->config['database'] : null);
	}
	...
	__wakeup() {
		throw
		new
		DibiNotSupportedException('You cannot serialize or unserialize ' . $this->getClass() . ' instances.');
	}
...
DibiResult
	...
	getResultDriver() {
		if ($this->driver === null) {
			throw
			new
			RuntimeException('Result-set was released from memory.');
		}
	...
	fetch() {
		...
		if ($this->rowFactory) {
			return
				call_user_func($this->rowFactory, $row);
		...
		foreach ($this->types
			...
			if ($value === false || $type === dibi::TEXT) {
				...
				if ((int) $value === 0) {
					...
					$row[$key] = new
					DibiDateTime(is_numeric($value) ? date('Y-m-d H:i:s', $value) : $value);
	...
	getType($col) {
		return
			isset($this->types[$col]) ? $this->types[$col] : null;
	}
	...
	getFormat($type) {
		return
			isset($this->formats[$type]) ? $this->formats[$type] : null;
	}
	...
	getInfo() {
		if ($this->meta === null) {
			$this->meta = new
			DibiResultInfo($this->getResultDriver());
		}
...
DibiRow
	...
	asDateTime($key, $format = null) {
		...
		if (!$time
			...
			$time = new
			DibiDateTime(is_numeric($time) ? date('Y-m-d H:i:s', $time) : $time);
		}
	...
	asTimestamp($key) {
		...
		return (int) $time === 0 ? null : (is_numeric($time) ? (int) $time : strtotime($time));
	}
...
DibiTranslator
	...
	translate(array$args) {
		...
		DibiHashMap(array($this, 'delimite'));
	...
	cb($matches) {
		...
		if ($matches[1]) return $this->identifiers->{$matches[1]};
		if ($matches[2]) return $this->identifiers->{$matches[2]};
		if ($matches[3]) return $this->driver->escape(str_replace("''", "'", $matches[4]), dibi::TEXT);
		if ($matches[5]) return $this->driver->escape(str_replace('""', '"', $matches[6]), dibi::TEXT);
		if ($matches[7]) {
			$this->hasError = true;
...
DibiFluent
	...
	public
	static $modifiers = array('SELECT' => '%n', 'FROM' => '%n', 'IN' => '%in', 'VALUES' => '%l', 'SET' => '%a', 'WHERE' => '%and', 'HAVING' => '%and', 'ORDER BY' => '%by', 'GROUP BY' => '%by');
	public
	static $separators = array('SELECT' => ',', 'FROM' => ',', 'WHERE' => 'AND', 'GROUP BY' => ',', 'HAVING' => 'AND', 'ORDER BY' => ',', 'LIMIT' => false, 'OFFSET' => false, 'SET' => ',', 'VALUES' => ',', 'INTO' => false);
	public
	static $clauseSwitches = array('JOIN' => 'FROM', 'INNER JOIN' => 'FROM', 'LEFT JOIN' => 'FROM', 'RIGHT JOIN' => 'FROM');
	private $connection;
	...
	__construct(DibiConnection $connection) {
		...
		if (self::$normalizer === null) {
			self::$normalizer = new
			DibiHashMap(array(__CLASS__, '_formatClause'));
		}
	}
	...
	__call($clause, $args) {
		...
		if (count($args) === 1) {
			...
			if ($arg === true) {
				return $this;
			} elseif (is_string($arg) && preg_match('#^[a-z:_][a-z0-9_.:]*\z#i', $arg)) {
				$args = array('%n', $arg);
	...
	toDataSource() {
		return
			new
			DibiDataSource($this->connection->translate($this->_export()), $this->connection);
	}
	...
	_formatClause($s) {
		...
		return
			strtoupper(preg_replace('#[a-z](?=[A-Z])#', '$0 ', $s));
	}

	function
	__clone() {
		foreach ($this->clauses
		         as $clause => $val) {
	...
	init() {
		if ($this->tables === null) {
			...
			foreach ($this->reflector->getTables() as $info) {
				...
				DibiTableInfo($this->reflector, $info);
			}
		}
	}
}
	...
	initColumns() {
		if ($this->columns === null) {
			...
			foreach ($this->reflector->getColumns($this->name) as $info) {
				...
				DibiColumnInfo($this->reflector, $info);
			}
		}
	}
...
DibiResultInfo
	...
	function
	__construct(IDibiResultDriver $driver) {
		$this->driver = $driver;
	}
	...
	initColumns() {
		if ($this->columns === null) {
			$this->columns = array();
			$reflector = $this->driver
			instanceof
			IDibiReflector ? $this->driver : null;
			foreach ($this->driver->getResultColumns() as $info) {
				$this->columns[] = $this->names[$info['name']] = new
				DibiColumnInfo($reflector, $info);
			}
		}
	}
}
...
DibiColumnInfo
	...
	getFullName() {
		return
			isset($this->info['fullname']) ? $this->info['fullname'] : null;
	}
	...
	getTable() {
		if (empty($this->info['table']) || !$this->reflector) {
			throw
			new
			DibiException("Table is unknown or not available.");
		}

		return
			new
			DibiTableInfo($this->reflector, array('name' => $this->info['table']));
	}

	function
	getTableName() {
		return
			isset($this->info['table']) ? $this->info['table'] : null;
	}

	function
	getType() {
		return
			self::getTypeCache()->{$this->info['nativetype']};
	}
	...
	isUnsigned() {
		return
			isset($this->info['unsigned']) ? (bool) $this->info['unsigned'] : null;
	}
	...
	function
	detectType($type) {
		static $patterns = array('^_' => dibi::TEXT, 'BYTEA|BLOB|BIN' => dibi::BINARY, 'TEXT|CHAR|POINT|INTERVAL' => dibi::TEXT, 'YEAR|BYTE|COUNTER|SERIAL|INT|LONG|SHORT' => dibi::INTEGER, 'CURRENCY|REAL|MONEY|FLOAT|DOUBLE|DECIMAL|NUMERIC|NUMBER' => dibi::FLOAT, '^TIME$' => dibi::TIME, 'TIME' => dibi::DATETIME, 'DATE' => dibi::DATE, 'BOOL' => dibi::BOOL);
		foreach ($patterns
		         as $s => $val) {
	...
	getTypeCache() {
		if (self::$types === null) {
			self::$types = new
			DibiHashMap(array(__CLASS__, 'detectType'));
		}
...
DibiEvent {
	const
		CONNECT = 1, SELECT = 4, INSERT = 8, DELETE = 16, UPDATE = 32, QUERY = 60, BEGIN = 64, COMMIT = 128, ROLLBACK = 256, TRANSACTION = 448, ALL = 1023;
	public $connection;
	public $type;
	public $sql;
	public $result;
...
DibiFirePhpLogger
	...
	private
	static $fireTable = array(array('Time', 'SQL Statement', 'Rows', 'Connection'));
	...
	logEvent(DibiEvent $event) {
		...
		$this->numOfQueries++;
		self::$fireTable[] = array(sprintf('%0.3f', $event->time * 1000), strlen($event->sql) > self::$maxLength ? substr($event->sql, 0, self::$maxLength) . '...' : $event->sql, $event->result
		instanceof
		Exception ? 'ERROR' : (string) $event->count, $event->connection->getConfig('driver') . '/' . $event->connection->getConfig('name'));
		...
		$payload = json_encode(array(array('Type' => 'TABLE', 'Label' => 'dibi profiler (' . $this->numOfQueries . ' SQL queries took ' . sprintf('%0.3f', $this->totalTime * 1000) . ' ms)'), self::$fireTable));
...
if (interface_exists('Nette\Diagnostics\IBarPanel') || interface_exists('IBarPanel')) {
	...
	DibiNettePanel
		...
		renderException($e) {
			if ($e
				instanceof
				DibiException && $e->getSql()
			) {
				return
					array('tab' => 'SQL', 'panel' => dibi::dump($e->getSql(), true));
			}
		}
...
dibi {
	const
		TEXT = 's', BINARY = 'bin', BOOL = 'b', INTEGER = 'i', FLOAT = 'f', DATE = 'd', DATETIME = 't', TIME = 't';
	const
		IDENTIFIER = 'n';
	const
		FIELD_TEXT = dibi::TEXT, FIELD_BINARY = dibi::BINARY, FIELD_BOOL = dibi::BOOL, FIELD_INTEGER = dibi::INTEGER, FIELD_FLOAT = dibi::FLOAT, FIELD_DATE = dibi::DATE, FIELD_DATETIME = dibi::DATETIME, FIELD_TIME = dibi::TIME;
	const
		VERSION = '2.1.0', REVISION = 'dc4fe8b released on 2013-06-23';
	const
		ASC = 'ASC', DESC = 'DESC';
	private
	...
	getSubstitutes() {
		return
			self::getConnection()->getSubstitutes();
	}
...
DibiMySqlReflector
	...
	getTables() {
		...
		while ($row = $res->fetch(false)) {
			$tables[] = array('name' => $row[0], 'view' => isset($row[1]) && $row[1] === 'VIEW');
		}
	...
	getColumns($table) {
		...
		while ($row = $res->fetch(true)) {
			...
			$columns[] = array('name' => $row['Field'], 'table' => $table, 'nativetype' => strtoupper($type[0]), 'size' => isset($type[1]) ? (int) $type[1] : null, 'unsigned' => (bool) strstr($row['Type'], 'unsigned'), 'nullable' => $row['Null'] === 'YES', 'default' => $row['Default'], 'autoincrement' => $row['Extra'] === 'auto_increment', 'vendor' => $row);
		}
...
DibiMySqlDriver
		...
		switch ($type) {
			...
			dibi::TEXT:
				if (!is_resource($this->connection)) {
					throw
					new
					DibiException('Lost connection to server.');
				}
	...
	getRowCount() {
		if (!$this->buffered) {
			throw
			new
			DibiNotSupportedException('Row count is not available for unbuffered queries.');
		}
	...
	seek($row) {
		if (!$this->buffered) {
			throw
			new
			DibiNotSupportedException('Cannot seek an unbuffered result set.');
		}
	...
	getResultResource() {
		...
		return
			is_resource($this->resultSet) ? $this->resultSet : null;
	}
}
...
DibiMySqliDriver
	...
	__construct() {
		if (!extension_loaded('mysqli')) {
			throw
			new
			DibiNotSupportedException("PHP extension 'mysqli' is not loaded.");
		}
	}
	...
	function
	query($sql) {
		$res = @mysqli_query($this->connection, $sql, $this->buffered ? MYSQLI_STORE_RESULT : MYSQLI_USE_RESULT);
	...
	getAffectedRows() {
		return
			mysqli_affected_rows($this->connection) === -1 ? false : mysqli_affected_rows($this->connection);
	}
...
DibiOdbcDriver
	...
	__construct() {
		if (!extension_loaded('odbc')) {
			throw
			new
			DibiNotSupportedException("PHP extension 'odbc' is not loaded.");
		}
	}
	...
	getInsertId($sequence) {
		throw
		new
		DibiNotSupportedException('ODBC does not support autoincrementing.');
	}
	...
	applyLimit(&$sql, $limit, $offset) {
		...
		if ($offset) throw
		new
		DibiNotSupportedException('Offset is not implemented in driver odbc.');
	}
	...
	fetch($assoc) {
		if ($assoc) {
			...
			$set = $this->resultSet;
			if (!odbc_fetch_row($set, ++$this->row)) return
				false;
	...
	getResultColumns() {
		...
		$columns = array();
		for ($i = 1; $i <= $count; $i++) {
			$columns[] = array('name' => odbc_field_name($this->resultSet, $i), 'table' => null, 'fullname' => odbc_field_name($this->resultSet, $i), 'nativetype' => odbc_field_type($this->resultSet, $i));
		}
	...
	getTables() {
		...
		while ($row = odbc_fetch_array($res)) {
			if ($row['TABLE_TYPE'] === 'TABLE' || $row['TABLE_TYPE'] === 'VIEW') {
				$tables[] = array('name' => $row['TABLE_NAME'], 'view' => $row['TABLE_TYPE'] === 'VIEW');
			}
		}
	...
	getColumns($table) {
		...
		while ($row = odbc_fetch_array($res)) {
			if ($row['TABLE_NAME'] === $table) {
				$columns[] = array('name' => $row['COLUMN_NAME'], 'table' => $table, 'nativetype' => $row['TYPE_NAME'], 'size' => $row['COLUMN_SIZE'], 'nullable' => (bool) $row['NULLABLE'], 'default' => $row['COLUMN_DEF']);
			}
		}
		odbc_free_result($res);
...
DibiSqliteReflector
	...
	getColumns($table) {
		...
		while ($row = $res->fetch(true)) {
			...
			$type = explode('(', $row['type']);
			$columns[] = array('name' => $column, 'table' => $table, 'fullname' => "$table.$column", 'nativetype' => strtoupper($type[0]), 'size' => isset($type[1]) ? (int) $type[1] : null, 'nullable' => $row['notnull'] == '0', 'default' => $row['dflt_value'], 'autoincrement' => (bool) preg_match($pattern, $meta['sql']), 'vendor' => $row);
		}
	...
	getIndexes($table) {
		...
		if (!$indexes) {
			foreach ($columns
			         as $column) {
				if ($column['vendor']['pk']) {
					$indexes[] = array('name' => 'ROWID', 'unique' => true, 'primary' => true, 'columns' => array($column['name']));
					break;
				}
			}
		}
...
DibiPdoDriver
	...
	__construct() {
		if (!extension_loaded('pdo')) {
			throw
			new
			DibiNotSupportedException("PHP extension 'pdo' is not loaded.");
		}
	}
	...
	applyLimit(&$sql, $limit, $offset) {
		...
		switch ($this->driverName) {
			...
			default:
				throw
				new
				DibiNotSupportedException('PDO or driver does not support applying limit or offset.');
		}
	}
	...
	getResultColumns() {
		...
		for ($i = 0; $i < $count; $i++) {
			...
			if ($row === false) {
				throw
				new
				DibiNotSupportedException('Driver does not support meta data.');
			}
...
DibiPostgreDriver
	...
	__construct() {
		if (!extension_loaded('pgsql')) {
			throw
			new
			DibiNotSupportedException("PHP extension 'pgsql' is not loaded.");
		}
	}
	...
	getTables() {
		...
		if ($version < 7.4) {
			throw
			new
			DibiDriverException('Reflection requires PostgreSQL 7.4 and newer.');
		}
		$res = $this->query("
			SELECT
				table_name AS name,
	...
	getColumns($table) {
		...
		while ($row = $res->fetch(true)) {
			...
			$columns[] = array('name' => $row['column_name'], 'table' => $table, 'nativetype' => strtoupper($row['udt_name']), 'size' => $size ? $size : null, 'nullable' => $row['is_nullable'] === 'YES', 'default' => $row['column_default'], 'autoincrement' => (int) $row['ordinal_position'] === $primary && substr($row['column_default'], 0, 7) === 'nextval', 'vendor' => $row);
		}
	...
	getForeignKeys($table) {
		...
		while ($row = $res->fetch(true)) {
			if (!isset($fKeys[$row['name']])) {
				$fKeys[$row['name']] = array('name' => $row['name'], 'table' => $row['table'], 'local' => array(), 'foreign' => array(), 'onUpdate' => $row['onUpdate'], 'onDelete' => $row['onDelete']);
...
DibiSqliteDriver
	...
	__construct() {
		if (!extension_loaded('sqlite')) {
			throw
			new
			DibiNotSupportedException("PHP extension 'sqlite' is not loaded.");
		}
	}
...
DibiSqlite3Driver
	...
	__construct() {
		if (!extension_loaded('sqlite3')) {
			throw
			new
			DibiNotSupportedException("PHP extension 'sqlite3' is not loaded.");
		}
	}
	...
	connect(array&$config) {
		...
		$this->fmtDate = isset($config['formatDate']) ? $config['formatDate'] : 'U';
		$this->fmtDateTime = isset($config['formatDateTime']) ? $config['formatDateTime'] : 'U';
		if (isset($config['resource']) && $config['resource'] instanceof
			SQLite3
		) {
			$this->connection = $config['resource'];
		} else
	...
	query($sql) {
		...
		if ($this->connection->lastErrorCode()) {
			throw
			new
			DibiDriverException($this->connection->lastErrorMsg(), $this->connection->lastErrorCode(), $sql);
		} elseif ($res
			instanceof
			SQLite3Result
		) {
	...
	getResultColumns() {
		...
		for ($i = 0; $i < $count; $i++) {
			$columns[] = array('name' => $this->resultSet->columnName($i), 'table' => null, 'fullname' => $this->resultSet->columnName($i), 'nativetype' => $types[$this->resultSet->columnType($i)]);
		}
